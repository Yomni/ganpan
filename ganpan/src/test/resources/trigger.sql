...
CREATE TRIGGER sb_sbname_org_trigger
...
CREATE TRIGGER sb_sbname_invite_trigger
AFTER UPDATE ON sign_board FOR EACH ROW
BEGIN
...
CREATE TRIGGER sb_have_trigger
AFTER UPDATE ON sign_board FOR EACH ROW
BEGIN
  UPDATE HAVE_BOARD
  SET sign_board_name=:NEW.sign_board_name
...
CREATE TRIGGER sb_bossnickname_invite_trigger
AFTER UPDATE ON sign_board FOR EACH ROW
BEGIN
  UPDATE invitation_management
...
CREATE TRIGGER sb_bossnickname_org_trigger
AFTER UPDATE ON sign_board FOR EACH ROW
BEGIN
  UPDATE organization
  SET boss_nickname=:NEW.boss_nickname
...
CREATE TRIGGER sb_bossnickname_have_trigger
AFTER UPDATE ON sign_board FOR EACH ROW
BEGIN
  UPDATE have_board
  SET boss_nickname=:NEW.boss_nickname
...
CREATE TRIGGER hb_bossnickname_work_trigger
...
CREATE TRIGGER org_bossnickname_work_trigger
